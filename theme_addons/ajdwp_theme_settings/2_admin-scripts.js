...
//--------------------------- Theme Settings Menu Tabs ---------------------------//
jQuery(function ($) {
  var KEY = "ajdwp_settings_active_tab";

  function activate(id) {
    $(".nav-tab-wrapper a").removeClass("nav-tab-active");
    $(".tab-content").removeClass("active");
    $('.nav-tab-wrapper a[href="#' + id + '"]').addClass("nav-tab-active");
    $("#" + id).addClass("active");
  }
  var init = (location.hash || "").replace("#", "") || localStorage.getItem(KEY) || "general";
  activate(init);
  ...
  $(".nav-tab-wrapper a").on("click", function (e) {
    ...
    history.replaceState(null, "", "?page=AJDWP_Theme_Options#" + id);
  });
});
